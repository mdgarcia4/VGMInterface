...
public class OrdenRestController {
	...
	//@PostMapping()
	@RequestMapping(value = RutasServicios.PEDIDO, method = RequestMethod.POST, produces=MediaType.APPLICATION_JSON_VALUE)
	...
	public String createPedido(@RequestBody String pedidoString) throws Exception {
		...
		try {
			...
			orders = orderService.tratar(ordersDef,"DEFINITIVA");
	...
	@RequestMapping(value = RutasServicios.PEDIDO_ACTUALIZA, method = RequestMethod.POST, produces=MediaType.APPLICATION_JSON_VALUE)
	@JsonView(Views.Response.class)
	public String actualizaPedido(@RequestBody String pedidoString) throws Exception {
		//Gson gson = new Gson();
		//Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss").create();
		//JSONObject jsonPedido = new JSONObject(pedidoString);
		//Orders orders = gson.fromJson(jsonPedido.toString(), Orders.class);
		...
		try {
			...
			LOG.info("Actualiza Pedido Json: " + pedidoString);
			//orders = orderService.tratar(ordersDef,"DEFINITIVA");
			//LOG.info("Pedido Definitivo creado OK " );
